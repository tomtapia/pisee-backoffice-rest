...
@Component
@Scope("singleton")
public class RestCplt {
	
	private final static Log LOGGER = LogFactory.getLog(RestCplt.class);	

	//FIXME: El metodo GET es solo para pruebas, en produccion debe ser POST por la cantidad de parametros
	@GET
//	@POST
	@Path("v1/enviaSolicitudPortal")
	...
	public String sendToPortalTransaparencia(@QueryParam("pisee_token") String piseeToken,
		...
		respuesta = restBusiness.callService(ConfigProveedoresServicios.SOAP_CPLT__PORTAL_TRANSPARENCIA, inputParameter);
		
		String value = jsonUtil.toJSON(respuesta);
		long endTime = System.currentTimeMillis();		
		LOGGER.info("sendToPortalTransaparencia CPLT - TIME == " + (endTime - startTime) + " MILISECONDS");
